...
$n = 4;
 ...
 $query = "SELECT otpVerified,device_id FROM tblusers WHERE mobile='$user_mobile'";
 $result=mysqli_query($con,$query);
 $resultRow = mysqli_fetch_assoc($result);

 $query2 = "SELECT mobile FROM tblusers WHERE mobile='$user_mobile'";
...
if($resultRow2['mobile']!=null){
 if($resultRow['otpVerified']=="yes" && $resultRow['device_id']==$device_id){
	  ...
	  if($query){
		 ...
		 $query = "SELECT * FROM tblusers WHERE mobile='$user_mobile'";  
		...
		$queryresult['error'] = 'User not exist';
	      ...
	      $query_up = "UPDATE `tblusers` SET `otpval` = '$otp' WHERE mobile='$user_mobile'";
